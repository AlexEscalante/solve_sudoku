...
My goal here is to reproduce a sudoku solver I wrote a year ago in Ruby. 

What bothers me is that, while it seems that I got a working Ruby implementation, it's clever enough to prove challenging to port to Javascript without hassle. 

In fact, my brute force, recursive backtracking search algorithm is complex and not easy to explain without resorting to the code… There should be a more concise, mathematical explanation and I certainly would like to find it.

There's a paper by Peter Norvig that presents an algorithm that resembles mine. Check the last one, not the first one:

http://norvig.com/sudoku.html

On top of that I want the solver to use Redux in order to put observers into play and see in real time how it works, with a React visualization of the board the whole time… and everything implemented in the most ES7 way possible!
